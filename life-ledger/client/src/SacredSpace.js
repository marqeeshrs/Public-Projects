...
function SacredSpace() {
    const [entry, setEntry] = useState("");
    const [lastSaved, SetLastSaved] = useState(null);

    //Load saved entry from local storage
    useEffect(() => {
        const saved = localStorage.getItem("sacred_entry");
        if (saved) {
            setEntry(saved);
        }
    }, []);
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            localStorage.setItem("sacred_entry", entry);
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-p50 text-white p-6">
            <h1 className="text-3x1 font-bold mb-4">Think Space</h1>
            <textarea
                value={entry}
                onChange={(e) => setEntry(e.target.value)}
                placeholder="Type your thoughts here... This is your space to organize your thoughts."
                className="w-full max-w-3x1 h-[400px] p-4 rounded-x1 bg-gray-900 text-white border border-gray-700 focus:outlin-none focus:ring-2 focus:ring-pink-500 resize-none shadow-lg"
            />
